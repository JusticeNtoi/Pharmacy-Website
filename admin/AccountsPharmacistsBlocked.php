  ...
  <body>
    ...
    <nav>
      ...
      <ul class="nav-links">
        ...
        <div class="header">
            <h1>PHARMACISTS ACCOUNTS BLOCKED</h1>
        </div>  
    ...
    echo "<tr><th> Pharmacist ID</th><th> Name</th><th> Surname</th><th> Email</th><th> Password</th><th>Block</th><th>Delete</th></tr>";
    while ($row =mysqli_fetch_assoc($result)) {
    echo "<tr><td> {$row['id']} </td><td> {$row['name']}</td><td> {$row['surname']}</td><td> {$row['email']}</td><td> {$row['password']}</td>
    <td><a href='PharmacistUnblock.php?id=$row[id]'>Unblock Account</a></td>
